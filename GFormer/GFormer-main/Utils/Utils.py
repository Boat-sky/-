...
def contrast(nodes, allEmbeds, allEmbeds2=None):
    if torch.any(torch.isnan(nodes)) or torch.any(torch.isinf(nodes)):
        raise ValueError("Nodes tensor contains NaN or infinity values.")
    
    # Ensure valid indices
    max_node = nodes.max().item()
    min_node = nodes.min().item()
    if max_node >= allEmbeds.size(0) or min_node < 0:
        raise ValueError(f"Invalid node indices: min = {min_node}, max = {max_node}")
