...
class BaseRunner(object):
	...
	def parse_runner_args(parser):
		...
		parser.add_argument('--eval_batch_size', type=int, default=16, #256
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		sub = model.generate_sub_adj()
		cmp = model.generate_cmp_adj()
		encoderAdj = model.generate_encoder_adj()
		...
		model.train()
		loss_lst = list()
		dl = DataLoader(subset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers,
						collate_fn=dataset.collate_batch, pin_memory=self.pin_memory)
		for i, batch in enumerate(tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1)):
			batch = utils.batch_to_gpu(batch, model.device)

			with autocast():  # Mixed precision training
				# Randomly shuffle the items to avoid models remembering the first item being the target
				item_ids = batch['item_id']  # Shape: [batch_size, seq_len]

				# Generate random indices for shuffling
				indices = torch.argsort(torch.rand(*item_ids.shape), dim=-1)  # Shape: [batch_size, seq_len]

				# Shuffle the `item_ids`
				shuffled_item_ids = item_ids[torch.arange(item_ids.shape[0]).unsqueeze(-1), indices]  # Shape: [batch_size, seq_len]
				batch['item_id'] = shuffled_item_ids

				out_dict = model(batch, is_test=False, sub=sub, cmp=cmp, encoderAdj=encoderAdj)
				loss = model.loss(out_dict) / accumulation_steps  # Scale loss

			scaler.scale(loss).backward()

			if (i + 1) % accumulation_steps == 0:
				scaler.step(model.optimizer)  # Optimizer step
				scaler.update()
				model.optimizer.zero_grad() 

			# Restore the shuffled predictions
			prediction = out_dict['prediction']  # Shape: [batch_size, seq_len, feature_dim]

			# Ensure the prediction aligns with the batch
			if prediction.shape[0] != item_ids.shape[0]:
				prediction = prediction[:item_ids.shape[0]]  # Slice to match batch size

			# Initialize a restored prediction tensor
			restored_prediction = torch.zeros_like(prediction)  # Shape: [batch_size, seq_len, feature_dim]

			# Restore predictions
			for i in range(len(indices)):
				restored_prediction[i, torch.arange(indices.shape[1])] = prediction[i, indices[i]]

			# Replace the restored prediction in `out_dict`
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
			if hasattr(dataset.model,'inference'):
				# Example initialization or derivation of the required arguments
				is_test = True  # Or derive this dynamically based on the context
				sub = dataset.model.generate_sub_adj()  # Replace with actual method to generate sub
				cmp = dataset.model.generate_cmp_adj()  # Replace with actual method to generate cmp
				encoderAdj = dataset.model.generate_encoder_adj()  # Replace with actual method to generate encoderAdj
				# Call the model with the required arguments
				prediction = dataset.model.inference(utils.batch_to_gpu(batch, dataset.model.device), is_test, sub, cmp, encoderAdj)['prediction']
				...
				# Call the model with the required arguments
				prediction = dataset.model(utils.batch_to_gpu(batch, dataset.model.device), is_test, sub, cmp, encoderAdj)['prediction']
