...
#from models.my_model.GFormer import GFormer 
...
def main():
	...
	# Read data
	# args.path: data/
	# args.dataset: Grocery_and_Gourmet_Food
	# model_name.reader: SeqReader
	# args.data_appendix: ไม่มี
	data_path = "C:/Users/ASUS/Downloads/lab_ML/big_homework/ori_ReChorus2/data"
	corpus_path = os.path.join(data_path, args.dataset, model_name.reader+args.data_appendix+ '.pkl')
	...
	for phase in ['train', 'dev', 'test']:
		...
		print(f"Phase: {phase}, Keys in dataset: {data_dict[phase].data.keys()}")
...
if __name__ == '__main__':
	#print(f"args.path:{args.path}, args.dataset:{args.dataset}, model_name.reader:{model_name.reader}, args.data_appendix:{args.data_appendix}")
	init_parser = argparse.ArgumentParser(description='Model')
	#init_parser.add_argument('--model_name', type=str, default='SASRec', help='Choose a model to run.')
	init_parser.add_argument('--model_name', type=str, default='GFormer', help='Choose a model to run.')
	#init_parser.add_argument('--model_mode', type=str, default='', 
	init_parser.add_argument('--model_mode', type=str, default='TopK', 
	...
	# ได้ string ออกมาคือ 'init_args.model_name'.'init_args.model_name''init_args.model_mode'
	#model_name = eval('{0}.{0}{1}'.format(init_args.model_name,init_args.model_mode))
	model_name = eval(f"{init_args.model_name}.{init_args.model_name}")
	...
	args, extras = parser.parse_known_args(namespace=init_args)
	...
	args.model_mode = init_args.model_mode
