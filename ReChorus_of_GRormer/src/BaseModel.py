...
class BaseModel(nn.Module):
	...
	class Dataset(BaseDataset):
		...
		def _build_adj_matrix(self):
			interactions = self.corpus.data_df['train'][['user_id', 'item_id']]
			rows = interactions['user_id'].values
			cols = interactions['item_id'].values + self.n_users
			data = np.ones_like(rows)
			adj_matrix = sp.csr_matrix((data, (rows, cols)), shape=(self.n_users + self.n_items, self.n_users + self.n_items))
			return adj_matrix
...
class GeneralModel(BaseModel):
	...
	class Dataset(BaseModel.Dataset):
		def _get_feed_dict(self, index):
			...
			#print("Available keys in self.data:", self.data.keys())
			#print("Index being accessed:", index)
			#print("Length of self.data['neg_items']:", len(self.data['neg_items']))
